...
def makedirectories(ip):
        if "http://" in ip:
                os.mkdir(ip.replace("http://",''))
                os.chdir(ip.replace("http://",''))
        elif "https://" in ip:
                os.mkdir(ip.replace("https://",''))  
                os.chdir(ip.replace("https://",''))
        else:
                os.mkdir(ip)
                os.chdir(ip)
makedirectories(ip)
#check whether or not it is valid request 
if "http" not in ip:
        ip = ip.strip('\n')
        ip = "http://" + ip.strip('\n')
else:
...
#confirmed its working working
r = requests.get(ip)
r = r.text
if r is not None:
        print("confirmed Host Website is up Starting Enumeration.")
else:
        print("Host Website is Down")
        sys.exit()
